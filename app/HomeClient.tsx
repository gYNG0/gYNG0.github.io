...
export default function HomeClient() {
  ...
  return <main className="app-shell" lang={language}>
    ...
    {screen !== "home" && screen !== "planner" && screen !== "search" && <section className="app-view">
      ...
      {screen === "discover" && <section className="discover-view"><p className="eyebrow dark">LOCAL FOOD DISCOVERY</p><h2>Near {place.name}</h2><p className="section-lead">Restaurant suggestions ordered by displayed Naver rating. Ratings are reference values—check Naver Map for the current score, hours and availability.</p><div className="location-bar"><div><b>GPS distance</b><span>{locationMessage}</span></div><div className="location-actions"><button onClick={requestLocation}>Use my location</button><button className="location-help" onClick={() => setLocationHelpOpen((open) => !open)}>🔒 Location help</button></div></div>{locationHelpOpen && <div className="location-help-panel"><b>Allow location access</b><p>In Chrome or Edge, select the site controls icon beside the address, choose Location, then select Allow. In the in-app browser, try opening this site in your regular browser if no permission control is shown.</p></div>}<div className="restaurant-grid">{place.restaurants.map((restaurant, index) => { const coordinates = restaurantCoordinates[restaurant.name]; const distance = userPosition && coordinates ? walkingDistance(userPosition, coordinates) : null; return <article key={restaurant.name}><span>0{index + 1}</span><div className="score">★ {restaurant.score}<small>Naver rating guide</small></div><h3>{restaurant.name}</h3><p>{restaurant.kind} · near {place.name}</p>{distance !== null && <p className="distance-info">{distance.toFixed(1)} km away · about {Math.max(1, Math.round(distance * 5))} min on foot</p>}<a target="_blank" rel="noreferrer" href={`https://map.naver.com/p/search/${encodeURIComponent(restaurant.name)}`}>View Naver Map photos & details →</a></article>; })}</div><button className="next-view" onClick={() => setScreen("plan")}>← Back to route & hospital</button></section>}
